...
It implements the `ProcessPluginDefinition` interface by reading the required information from the `plugin.properties` file located in `tutorial-process/src/main/resources/plugin.properties`.
The DSF BPE server then searches for classes implementing this interface using the
Java [ServiceLoader](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/ServiceLoader.html) mechanism. Therefore, you will have to register your interface implementation in the `tutorial-process/src/main/resources/META-INF/services/dev.dsf.bpe.ProcessPluginDefinition` file.
